...
# Sub functions
# Processing data step 2 - LTR_Harvest & LTR_Digest
# Generate Gff3 annotation file for hmmsearch, this GFF3 file can be accepted as LTR_digest output
# ------------------------------------------------------------------------------------------------------------------
sub genGFF3 {
	...
	# convert RT info to predicted genome loci
	# genome = 5' 10000 bp + RT + 3' 10000bp
	open($input, "<", $rt);
	my @rt_seq = <$input>;
	shift @rt_seq;
	...
	# print GFF3 file header and parental record
	...
	while (defined($_ = <$input>)) {
		...
		# convert ORF frame shift to standard order
		if ($frame =~ /1|2|3/) { $strand = "+"; $frame -= 1; }
		else { $strand = "-"; $frame -= 4; }
		# print GFF3 feature discription
...
sub step2_summarize_annotation {
	...
	for (my $i = 0; $i < @reps; $i ++) {
		# clean all pervious elements for new records
		undef @ltrs;
		undef @pros;
		undef @LTR_loci;
		@lines = split("\n", $reps[$i]);
		# 
		foreach (@lines) {
			# Repeat region shows the range of repeat loci
			if    ($_ =~ /\t(repeat|target)_region\t(\d+)\t(\d+)\t/) { $region = $_; }
			# long terminal repeat feature
			...
			# provirus feature
			elsif ($_ =~ /\tLTR_retrotransposon\t(\d+)\t(\d+)/)      { $header = substr($header, 0, 20)."_$1\_$2"; }
		}

		# Summairse all information
		# provirus repeat region summary
		@_ = split("\t", $region);
		$reps[$i] = "$_[3],$_[4]";
		# LTR location summary
		# the cooridents of LTR is predicted by LTR_Harvest, if LTR cannot be found, use "0" instead
		...
		# LTR type is annotated according to Repbase Update, if LTR cannot be found, use "none" instead
		...
		# Primer binding site
		# Annotation of primer binding site according to the tRNAdb by LTR_Harvest
		# if LTR cannot be found, use "none" instead
		if (defined($pbs)) {
			@_ = split("\t", $pbs);
		    $reps[$i] .= ",$_[3],$_[4],";
		    @_ = split(";", $pbs);
		...
		# common gene feature annotation
		$reps[$i] .= ",".(scalar @pros);
		foreach (@pros) {
			@_ = split("name=", $_);
			$reps[$i] .= ",$_[-1]";
...
# Sub functions
# Processing data step 2 - LTR_Harvest & LTR_Digest
# blast LTRHarvest-predicted LTRs against Repbase Update database for LTR type annotation
# convert blast result into annotation line
# ------------------------------------------------------------------------------------------------------------------
sub blast_result {
